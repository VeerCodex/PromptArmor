...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiHost}/auth/login-json`, {
        method: "POST",
      ...
      if (err.message.includes("Failed to fetch")) {
        ...
        setError("Unable to reach FastAPI backend server. Make sure it is running on " + getApiUrl());
      } else {
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-900/40 border border-gray-800 rounded-2xl p-8 shadow-2xl backdrop-blur-md relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white tracking-tight">Sign in to PromptArmor</h2>
          <p className="text-xs text-gray-500 mt-1">Protecting AI applications in production</p>
        </div>
